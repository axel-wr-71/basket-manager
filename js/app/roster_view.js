...
export function renderRosterView(team, players) {
    ...
    let html = `
        <div class="roster-management-header" style="padding: 20px; display: flex; justify-content: space-between; align-items: center;">
            <div>
                ...
                <p style="margin:0; color:#64748b;">Current squad: <strong style="color:#1a237e">${team?.team_name || 'Twoja Drużyna'}</strong> | League: <strong style="color:#1a237e">${team?.league_name || 'Super League'}</strong></p>
            </div>
        ...
        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px; padding: 0 20px 30px 20px;">
            ${topStars.map((star, idx) => {
                // Korzystamy z globalnej funkcji potencjału
            ...
            <table style="width: 100%; border-collapse: collapse;">
                <thead>
                    <tr style="text-align: left; color: #94a3b8; font-size: 0.7rem; text-transform: uppercase; letter-spacing: 1px; border-bottom: 1px solid #f1f5f9;">
                        ...
                        <th style="padding: 20px;">HT (cm/ft)</th>
    ...
    // Obsługa kliknięć w przycisk Profile
...
function renderPlayerRow(p) {
    ...
    // PUNKT 2: Logika przeliczania wzrostu
        ...
        <tr style="border-bottom: 1px solid #f8fafc; vertical-align: middle;">
                    ...
                    <div style="flex: 1;">
                        ...
                        <div style="display: grid; grid-template-columns: repeat(3, 1fr); gap: 15px; background: #f8fafc; padding: 10px; border-radius: 10px; font-size: 0.65rem; border: 1px solid #edf2f7; min-width: 350px;">
                            <div>
                                ...
                                <div style="display:flex; justify-content:space-between; margin:2px 0;"><span>2PT Shot</span> <strong>${p.skill_2pt ?? '-'}</strong></div>
            ...
            <td style="padding: 20px; text-align: right;">
                <div style="display: flex; gap: 6px; justify-content: flex-end;">
                    <button class="btn-profile-trigger" data-id="${p.id}" style="background: #1a237e; color: white; border: none; padding: 8px 10px; border-radius: 6px; font-weight: 800; cursor: pointer; text-transform: uppercase; font-size: 0.6rem; min-width: 60px;">
                        Profile
                    </button>
                    <button style="background: #f1f5f9; color: #1a237e; border: 1px solid #e2e8f0; padding: 8px 10px; border-radius: 6px; font-weight: 800; cursor: pointer; text-transform: uppercase; font-size: 0.6rem; min-width: 60px;">
                        Train
                    </button>
                    <button style="background: white; color: #ef4444; border: 1px solid #fee2e2; padding: 8px 10px; border-radius: 6px; font-weight: 800; cursor: pointer; text-transform: uppercase; font-size: 0.6rem; min-width: 60px;">
                        Sell
                    </button>
                </div>
            </td>
